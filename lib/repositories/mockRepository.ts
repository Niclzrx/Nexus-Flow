...
// Estado em memória — reinicia a cada reload do servidor/dev server.
...
export class MockRepository implements FinanceRepository {
  ...
  async updateTransaction(input: UpdateTransaction): Promise<Transaction> {
    ...
    if (!updated) throw new Error(`Transação ${input.id} não encontrada`);
    return updated;
  }
  ...
  async updateGoal(input: UpdateGoal): Promise<Goal> {
    ...
    if (!updated) throw new Error(`Meta ${input.id} não encontrada`);
    return updated;
  }
  ...
  async addCategory(nome: string): Promise<Category> {
    ...
    if (exists) throw new Error(`A categoria "${nome}" já existe`);
